...
const ignoreLayout = ["/login", "/logout"];
...
const logoutFromAuth = async () => {
  const access_token = localStorage.getItem("user")
    ...
    : undefined;
  await auth
    .logout_from_server(access_token)
    .then((response) => {
...
const oidcConfig = {
  onSignIn: async (user) => {
    ...
    window.location.href = process.env.PUBLIC_URL
      ? `${process.env.PUBLIC_URL}${next || "/perfil"}`
      : `${next || "/perfil"}`;
  },
  onSignOut: async () => {
    logoutFromAuth();
...
const PrivateRoute = ({ component: Component, ...rest }) => {
  if (
    ...
    rest.location.pathname != "/login-empresa" &&
    rest.location.pathname != "/logout-empresa"
  ) {
